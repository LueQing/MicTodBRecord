...
- 当音量超过阈值（默认 `-20 dBFS`）时，自动保存该段录音（前后各 5 秒缓冲）并写入结构化日志
...
- `LOUD_CAPTURE_ENABLED`：`1`/`0`，默认 `1`
- `LOUD_THRESHOLD_DB`：触发阈值，默认 `-20`
- `LOUD_CAPTURE_BUFFER_MS`：前后缓冲毫秒，默认 `5000`
- `LOUD_CAPTURE_DIR`：输出根目录，默认 `captures`
...
- 高音量事件文件默认写入 `captures/`：
  - `captures/recordings/<datetime>.wav`
  - `captures/logs/<datetime>.json`
